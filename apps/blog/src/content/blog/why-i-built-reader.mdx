...
faq:
  - question: "What is Reader?"
    answer: "Reader is an open-source web scraping platform built for AI. It converts any webpage into clean markdown using two primitives: scrape for single URLs and crawl for entire websites. It handles JavaScript rendering, anti-bot bypass, and content extraction automatically."
  - question: "Is Reader free to use?"
    answer: "Reader is open-source and free to self-host. The managed API at reader.dev offers a free tier with 1,000 credits per month. The engine, SDKs, and CLI are all available on GitHub."
  - question: "What programming languages does Reader support?"
    answer: "Reader has official SDKs for JavaScript and Python, plus a CLI tool. The API works with any language that can make HTTP requests. The scraping engine itself is built on Node.js with a Rust-based markdown converter."
draft: false
---

Web scraping for AI should not require weeks of infrastructure work. But if you have built anything with LLMs that needs web data, you know it does.
...
The HTML comes back as a mess of ads, navigation, cookie banners, and tracking scripts. The actual content, the part you care about, is buried somewhere in the middle. So you write a parser. Then you discover half the web is rendered by JavaScript, so your parser sees nothing. You spin up a headless browser. Now you're managing Puppeteer, dealing with memory leaks, and watching your costs climb.
...
Reader is [open-source](/open-source) web scraping for LLMs. Two primitives: `scrape()` for URLs, `crawl()` for websites. Everything else happens under the hood.

[**Scrape**](/scrape) turns any URL into clean markdown:
...
[**Crawl**](/crawl) handles entire websites:
...
Reader is built on Playwright with production-grade infrastructure:
...
_Questions? Check the [documentation](https://docs.reader.dev) or open an issue on [GitHub](https://github.com/vakra-dev/reader). If you want to see how Reader compares to other tools, read our [web scraping API comparison](/blog/best-web-scraping-api) or learn how Reader [converts any website to clean markdown](/blog/convert-website-to-markdown)._
